...
func NewFpsCamera(position, worldUp mgl32.Vec3, yaw, pitch float64, im *InputManager) *FpsCamera {
	cam := FpsCamera{
		moveSpeed:         5.00,
...
func (c *FpsCamera) updatePosition(dTime float64) {
	...
	if c.inputManager.IsActive(PLAYER_FORWARD) {
		c.pos = c.pos.Add(fixedFront.Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(PLAYER_BACKWARD) {
		c.pos = c.pos.Sub(fixedFront.Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(PLAYER_LEFT) {
		c.pos = c.pos.Sub(c.front.Cross(c.up).Normalize().Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(PLAYER_RIGHT) {
